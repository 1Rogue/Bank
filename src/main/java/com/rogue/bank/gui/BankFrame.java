...
public class BankFrame extends JFrame {
    ...
    private ArrayList<String> loadAccounts() {
        ...
        for (Account acc : accounts) {
            strings.add(String.format("%s %s $%.2f", acc.getID(), acc.getDisplayString(), acc.getBalance()));
            i++;
        }
        return strings;
    }
    ...
    private void refreshAccounts() {
        for (int i = 0; i < this.accStrings.size(); i++) {
            ...
            this.accStrings.set(i, String.format("%s %s $%.2f", acc.getID(), acc.getDisplayString(), acc.getBalance()));
        }
